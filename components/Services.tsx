...
export function Services() {
  return (
    <section id="solucoes" className="relative bg-background py-16 sm:py-28 lg:py-36">
      <div className="container-premium">
        <motion.div
          ...
          className="mb-8 flex flex-col justify-between gap-5 md:mb-16 md:flex-row md:items-end"
        >
          <div className="max-w-2xl">
            <p className="eyebrow mb-5">Soluções sob medida</p>
            <h2 className="text-balance font-display text-3xl leading-[1.08] tracking-tight text-foreground sm:text-5xl lg:text-6xl">
        ...
        <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
          {SERVICES.map((service, index) => (
            <motion.a
              ...
              className="group relative block h-[300px] overflow-hidden border border-foreground/10 sm:h-[480px]"
              ...
              <div className="absolute inset-x-0 bottom-0 p-5 sm:p-8">
                <div className="flex items-end justify-between gap-5">
                  <div>
                    ...
                    <h3 className="mt-2 font-display text-xl text-cream sm:text-3xl">{service.title}</h3>
                    <p className="mt-3 hidden max-w-md text-sm leading-6 text-cream/65 sm:block">{service.description}</p>
                  </div>
                  <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-cream/30 text-cream transition-all duration-500 group-hover:-translate-y-1 group-hover:translate-x-1 group-hover:bg-cream group-hover:text-ink sm:h-12 sm:w-12">
