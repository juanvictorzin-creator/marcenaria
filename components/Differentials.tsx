...
export function Differentials() {
  return (
    <section id="diferenciais" className="bg-cream py-16 text-ink sm:py-28 lg:py-36">
      <div className="container-premium grid items-stretch gap-10 lg:grid-cols-12 lg:gap-16">
        <Reveal className="lg:col-span-5">
          <div className="relative h-full min-h-[320px] overflow-hidden rounded-t-[6rem] rounded-br-2xl sm:min-h-[480px] sm:rounded-t-[9rem] sm:rounded-br-3xl">
            <Image
        ...
        <div className="lg:col-span-7 lg:py-5">
          <Reveal>
            ...
            <h2 className="text-balance max-w-2xl font-display text-3xl leading-[1.08] tracking-tight sm:text-5xl lg:text-6xl">
              Feito para caber no espaço — e fazer sentido na sua rotina.
            </h2>
            <p className="mt-4 max-w-xl text-sm leading-6 text-ink/65 sm:mt-6 sm:text-base sm:leading-7">
          ...
          <div className="mt-7 grid grid-cols-2 gap-px overflow-hidden border border-ink/15 bg-ink/15 sm:mt-10">
            {DIFFERENTIALS.map((item, index) => (
              <Reveal key={item.title} delay={index * 0.06} className="h-full">
                <article className="group flex h-full flex-col bg-cream p-4 transition-colors duration-500 hover:bg-paper sm:min-h-56 sm:p-8">
                  ...
                  <h3 className="mt-5 font-display text-base leading-snug sm:mt-8 sm:text-xl">{item.title}</h3>
                  <p className="mt-3 hidden text-sm leading-6 text-ink/60 sm:block">{item.description}</p>
                </article>
              </Reveal>
            ))}
          </div>
